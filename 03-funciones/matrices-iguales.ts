...
function intercambiar(matrizX): void{
    let fila=[];
    let columna=[];
...
function verficar(valor,matrizX,matrizY): void{
    let validar=0;
    if(valor==1){
        for(let contadorI=0;contadorI<matrizX.length;contadorI++){
            for(let contadorJ=0;contadorJ<matrizX[0].length;contadorJ++){
...
function verificarNM(matrizX,matrizY): number{
